package no.kantega.robomadness;
...
import no.kantega.robomadness.broker.ConsumerRecord;
import no.kantega.robomadness.broker.ProducerRecord;
import no.kantega.robomadness.broker.TopicName;
...
import static no.kantega.robomadness.broker.ProducerRecord.*;
import static no.kantega.robomadness.broker.TopicName.*;
import static no.kantega.robomadness.Util.*;
...
public class Client {
    ...
    public static void run(WS ws, String id, String subscribe, Stream<String, String> app) {
        Stream<ConsumerRecord, ProducerRecord> wrapped =
          incoming -> incoming.map(rec -> rec.msg).through(app).map(toMessage(gameevent(id)));
    ...
    public static void run(WS ws, String id, Source<String> output) {
        run(ws, output.map(toMessage(gameevent(id))));
    }
