...
public class PersistentQueue implements Sink<ProducerRecord> {
    ...
    static final Path defaultLogDir =
            Paths.get(System.getProperty("user.home") + "/data").toAbsolutePath();
    ...
    //The log is placed in ~/data/logname
    public static <A> PersistentQueue log(String logname, ExecutorService executor) {
        return new PersistentQueue(defaultLogDir.resolve(logname), executor);
    }
